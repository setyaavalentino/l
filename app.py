...
def main():
    ...
    with col1:
        ...
        # Selection mode: Upload or Internal Folder
        selection_mode = st.radio("Source Mode", ["Internal Folder", "Upload File"])
        
        main_video_path = None
        if selection_mode == "Internal Folder":
            video_folder = "downloads/videos"
            if not os.path.exists(video_folder):
                os.makedirs(video_folder, exist_ok=True)
            
            files = [f for f in os.listdir(video_folder) if f.lower().endswith(('.mp4', '.mkv', '.mov', '.avi'))]
            if files:
                selected_file = st.selectbox("Select Video File", files)
                main_video_path = os.path.join(video_folder, selected_file)
            else:
                st.warning(f"No video files found in {video_folder}")
        else:
            uploaded_file = st.file_uploader("Upload Video", type=['mp4', 'mkv', 'mov', 'avi'])
            if uploaded_file:
                main_video_path = os.path.join("downloads/videos", uploaded_file.name)
